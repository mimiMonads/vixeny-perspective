...
type plugin = {
    type: 'compile' | 'compileFile' | 'compileClient' | 'compileClientWithDependenciesTracked' | 'compileFileClient' | 'render' | "renderFile"
...
const pugRenderer = () => {
    ...
    return {
        ...
        f: (_) => () => compileFile as (path: string, options?: Options)=> CompileFunction
    }
};

// Self-invoking function to enforce type-checking against the CyclePlugin type.
((I: CyclePlugin) => I)(pugRenderer());
